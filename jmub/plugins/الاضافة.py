...
async def lol(event):
    ...
    try:
        await jmub(functions.channels.JoinChannelRequest(bol))
        await event.edit("**- تم الانضمام بنجاح  ✓**")
    except Exception as e:
        await event.edit(str(e))


@jmub.ar_cmd(pattern="اضافه ([\s\S]*)")
async def _(event):
...
@jmub.ar_cmd(pattern="ضيف ([\s\S]*)", groups_only=True)
async def get_users(event):
    legen_ = event.text[10:]
    jmub_chat = legen_.lower
    restricted = ["@QW_PN", "@QW_PN"]
    jmub = await edit_or_reply(event, f"**جارِ اضأفه الاعضاء من  ** {legen_}")
    if jmub_chat in restricted:
    ...
    if not sender.id == me.id:
        await jmub.edit("**▾∮ تتم العملية انتظر قليلا ...**")
    else:
        await jmub.edit("**▾∮ تتم العملية انتظر قليلا ...**")
    if event.is_private:
        return await jmub.edit("- لا يمكنك اضافه الاعضاء هنا")
    s = 0
    f = 0
    error = "None"
    await jmub.edit("**▾∮ حالة الأضافة:**\n\n**▾∮ تتم جمع معلومات المستخدمين 🔄 ...⏣**")
    async for user in event.client.iter_participants(event.pattern_match.group(1)):
        try:
            ...
            tol = f"@{user.username}"
            lol = tol.split("`")
            await jmub(InviteToChannelRequest(channel=event.chat_id, users=lol))
            s = s + 1
            await jmub.edit(
                f"**▾∮تتم الأضافة **\n\n• اضيف `{s}` \n•  خطأ بأضافة `{f}` \n\n**× اخر خطأ:** `{error}`"
            )
