...
async def upstream(event):
    ...
    try:
        txt = "فشل في التحديث لسورس القرش " + "**• حدث خطأ ما :**\n"
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"**• ارسل** `{cmdhd}تحديث التنصيب` لتحديث سورس القرش"
        )

    if force_update:
        await event.edit("**• جار التحديث الاجباري الى اخر اصدار انتظر قليلا**")
    if conf == "الان":
        await event.edit("**• جار تحديث سورس القرش أنتظر قليلا**")
...
async def upstream(event):
    ...
    off_repo = "https://github.com/omar1211/Z"
    os.chdir("/app")
    try:
